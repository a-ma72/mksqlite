...
```
## Prerequisites
*mksqlite* is build on top of SQLite and tested on various MATLAB(R) versions (R13SP1 .. R2024b).
...
The Microsoft Visual C++ Compiler version 2003 or newer is needed to compile *mksqlite* on windows systems.
...
6. The file `mksqlite.mexmaci64` or `mksqlite.mexmaca64` will be in your `build` directory. Copy it to the `mksqlite` directory. 
